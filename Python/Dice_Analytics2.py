...
def read(file_path):
    if os.path.exists(file_path):
        ...
        data = [['Name', 'Rolls', 'Sum', 'Average', 'Mode', 'Rank']]
    return data

def get_rankings(data):
    data = sorted(data[1:], key=lambda x: x[2], reverse=True)
...
def main():
    ...
    ranked_data = get_rankings(data)
    
    print("}---------- RESULTS ----------{")
    print(" || {:<20} ".format("Name")," || {:<30} ".format("Rolls")," || {:<15} ".format("Sum"),
          " || {:<15} ".format("Average")," || {:<15} ".format("Mode"), " || {:<5} ".format("Rank"))
    
    for row in ranked_data:
        print(" || {:<20} ".format(str(row[0]))," || {:<30} ".format(str(row[1])),
              " || {:<15} ".format(str(row[2]))," || {:<15} ".format(str(row[3])),
              " || {:<15} ".format(str(row[4]))," || {:<5} ".format(str(row[5])))
